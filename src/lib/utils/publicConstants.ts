...
export const PUBLIC_CLOUDFLARE_TURNSTILE_SITE_KEY = pubEnv.PUBLIC_CLOUDFLARE_TURNSTILE_SITE_KEY || null;
if (!PUBLIC_CLOUDFLARE_TURNSTILE_SITE_KEY) {
	console.error("PUBLIC_CLOUDFLARE_TURNSTILE_SITE_KEY is required");
}

export const ALLOWED_SOCIAL_PROVIDERS = ['passkey', 'anonymous', 'google', 'github', 'discord', 'microsoft', 'linkedin', 'facebook', 'gitlab', 'huggingface', 'notion', 'reddit', 'slack', 'spotify', 'twitter', 'twitch', 'vercel'] as const;
