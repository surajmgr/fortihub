import { uniqueUsernameGenerator, type Config, adjectives, nouns, generateFromEmail } from "unique-username-generator";
...
export async function generateUsername(email?: string, tries = 0): Promise<string | undefined> {
    ...
    if (email && tries < 3) {
        ...
        const slicedMail =
            email.slice(0, email.indexOf("@") > -1 ? email.indexOf("@") : email.length).slice(0, MAX_LENGTH - digits) +
            "@gmail.com";
