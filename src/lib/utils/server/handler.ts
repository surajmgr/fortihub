...
export function dbAsyncHandler<TArgs, TResult>(
	dbOperation: (args: TArgs) => Promise<TResult>,
...
export function apiAsyncHandler<TArgs extends any[], TResult>(
	...
	return async (...args: TArgs): Promise<TResult> => {
		try {
			...
			const status =
				error instanceof ApiError ? error.code || error.statusCode || defaultStatus : defaultStatus;
