interface EmailOTPTemplateProps {
  ...
  title: string;
  url: string; // for logo src
}

export function generateEmailOTPTemplate({ otp, type, title, url }: EmailOTPTemplateProps) {
...
// Helper functions
function getPreviewText(type: "sign-in" | "email-verification" | "forget-password", otp: string): string {
  switch (type) {
    case "sign-in":
...
function getDescription(type: "sign-in" | "email-verification" | "forget-password"): string {
  switch (type) {
    case "sign-in":
